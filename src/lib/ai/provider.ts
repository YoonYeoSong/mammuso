...
function applySpicyTone(result: DecisionResult): DecisionResult {
  ...
  const openings = [
    "이번 건은 말이 아니라 행동에서 먼저 선을 넘었습니다.",
    "이렇게 처리하면 상대가 빡치는 게 당연한 상황입니다.",
    "감정 핑계로 넘기기엔 너무 답답하게 꼬아 놓았습니다.",
  ];
  const closings = [
    "다음엔 같은 식으로 뭉개지 말고, 문제된 행동부터 인정하는 편이 낫습니다.",
    "억울함을 말하기 전에 상대가 지적한 행동을 먼저 바로잡아야 합니다.",
    "결국 핵심은 감정이 아니라 실제로 누가 무엇을 했는지입니다.",
  ];
