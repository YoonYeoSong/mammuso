...
class GroqAIProvider implements AIProvider {
  ...
  generateJointDecision(input: { applicantStatement: string; applicantAnswers: Answers; respondentStatement: string; respondentAnswers: Answers; spicyMode: boolean }) { return this.decision(`양측 관계분쟁 진술을 비교해 조정 의견을 작성하세요. 제공되지 않은 사실을 만들지 말고 불명확하면 판단불가로 적으세요. 이는 법률 판단이 아닙니다. 신청인 진술: ${input.applicantStatement}\n신청인 답변:${JSON.stringify(input.applicantAnswers)}\n상대방 진술:${input.respondentStatement}\n상대방 답변:${JSON.stringify(input.respondentAnswers)}`, input.spicyMode); }
  generateAppealDecision(input: { applicantStatement: string; applicantAnswers: Answers; respondentStatement: string; respondentAnswers: Answers; previousResult: DecisionResult; appealText: string; spicyMode: boolean }) { return this.decision(`아래 관계분쟁을 재심의하세요. 기존 결과, 양측 진술, 새 이의신청만 근거로 하며 새 사실을 만들지 마세요. changedReason에는 기존 결과에서 달라진 이유 또는 변경 없음 사유를 쓰세요. 신청인:${input.applicantStatement}\n신청인답변:${JSON.stringify(input.applicantAnswers)}\n상대방:${input.respondentStatement}\n상대방답변:${JSON.stringify(input.respondentAnswers)}\n기존:${JSON.stringify(input.previousResult)}\n이의신청:${input.appealText}`, input.spicyMode); }
  private decision(context: string, spicyMode: boolean) { const tone = spicyMode ? "양측이 명시적으로 동의한 쌍욕 모드입니다. overview, reasoning, mediationAdvice 각각에 이 사건의 행동을 향한 강한 욕설·비속어를 최소 두 번씩 반복해 넣으세요. 완곡하게 돌려 말하지 말고, 짧고 세게 여러 번 말하세요. 다만 신체적 위협, 폭력 조장, 성적 모욕, 외모 비하, 보호 특성에 대한 혐오·비하 표현은 절대 쓰지 마세요." : "욕설이나 모욕적 표현 없이 중립적으로 작성하세요."; return this.ask(`${context}\n\n양측 진술만 근거로 이 사건의 짧은 조정 결과를 작성하세요. 제공된 그 상황만 다루고 일반적인 관계 조언, 인물 성격 해석, 역할극, 인사말을 덧붙이지 마세요. overview는 결론 1~2문장, agreedFacts와 disputedFacts는 각각 최대 2개, reasoning은 판단 이유 1~2문장, mediationAdvice는 이 사건에서 두 사람이 다음에 할 행동 1~2문장으로 작성하세요. claims와 unknownFacts도 최대 2개로 짧게 작성하되 추측하지 마세요. 책임지표는 갈등을 키운 정도이며, 단순한 관계 다툼에서 한쪽을 0 또는 100으로 두지 마세요. 불확실하면 50:50에 가깝게, 차이가 있으면 근거가 있는 범위에서 20~80 사이로 제시하세요. clerkComment와 changedReason에도 이 사건과 무관한 문구를 쓰지 마세요. ${tone}\nSchema: {"overview":"","agreedFacts":[],"complainantClaims":[],"respondentClaims":[],"disputedFacts":[],"unknownFacts":[],"complainantResponsibility":50,"respondentResponsibility":50,"reasoning":"","mediationAdvice":"","clerkComment":"","changedReason":""}`, decisionSchema); }
}
