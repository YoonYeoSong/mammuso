...
export interface CaseRepository {
  ...
  saveRespondentStatement(token: string, statement: string, answers: Answers, result: DecisionResult): Promise<MammusoCase>;
...
class SupabaseCaseRepository implements CaseRepository {
  ...
  saveRespondentStatement(token: string, statement: string, answers: Answers, result: DecisionResult) { return this.update(`respondent_token_hash=eq.${hashToken(token)}`, { respondent_statement: statement, respondent_answers: answers, final_result: result, status: "COMPLETED" }).then((v) => v!); }
