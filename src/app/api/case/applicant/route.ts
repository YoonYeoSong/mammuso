...
export async function POST(request: NextRequest) { try { const { token, answers } = schema.parse(await request.json()); assertNoSensitiveIdentifier(Object.values(answers)); const repo = getCaseRepository(); const item = await repo.getByApplicantToken(token); if (!item) throw new Error("CASE_NOT_FOUND"); if (classifySafety(Object.values(answers).join(" ")) === "urgent") return NextResponse.json({ case: await repo.markApplicantSafety(token, answers) }); const result = await getAIProvider().generatePreliminaryOpinion({ statement: item.complainantStatement, answers }); return NextResponse.json({ case: await repo.saveApplicantReview(token, answers, result, []) }); } catch (error) { return apiError(error); } }
