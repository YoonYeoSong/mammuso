...
export async function POST(request: NextRequest) { try { const { token, answers } = schema.parse(await request.json()); assertNoSensitiveIdentifier(Object.values(answers)); const repo = getCaseRepository(); const item = await repo.getByApplicantToken(token); if (!item) throw new Error("CASE_NOT_FOUND"); const savedAnswers = { ...answers, [SPICY_MODE_CONSENT_KEY]: item.complainantAnswers[SPICY_MODE_CONSENT_KEY] ?? "미동의" }; if (classifySafety(Object.values(answers).join(" ")) === "urgent") return NextResponse.json({ case: await repo.markApplicantSafety(token, savedAnswers) }); const result = await getAIProvider().generatePreliminaryOpinion({ statement: item.complainantStatement, incidentDate: item.incidentDate ?? "날짜 미상", answers: savedAnswers }); return NextResponse.json({ case: await repo.saveApplicantReview(token, savedAnswers, result, []) }); } catch (error) { return apiError(error); } }
