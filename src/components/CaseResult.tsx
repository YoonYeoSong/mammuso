...
export function CaseResult({ result, rehearing = false, spicyMode = false, respondentStatement }: { result: DecisionResult; rehearing?: boolean; caseNumber?: string; spicyMode?: boolean; respondentStatement?: string | null }) {
  ...
  return <section className="result-card">
    ...
    <section className="result-details">
      ...
      {showDetails && <div id={detailsId} className="result-details-content"><ResultList title="확인된 내용" items={result.agreedFacts} /><ResultList title="서로 다른 내용" items={result.disputedFacts} /><section><h3>판단 이유</h3><p>{result.reasoning}</p></section>{result.changedReason && <section><h3>달라진 점</h3><p>{result.changedReason}</p></section>}{spicyMode && respondentStatement && <details className="respondent-original"><summary>상대방 원문 보기</summary><p>{respondentStatement}</p></details>}</div>}
    </section>
