...
export function DailyHeartLetter() {
  ...
  return <section className={`daily-letter ${isOpen ? "is-open" : ""}`} aria-labelledby="daily-letter-title">
    ...
    {isOpen && <div id="daily-letter-message" className="daily-letter-message" role="status"><p className="eyebrow">오늘 김햄찌의 한마디</p><p>{message}</p><div><Link href="/intake" className="button">이 마음으로 사연 정리하기 <span>→</span></Link><a href="#process" className="text-button">민원 절차 보기</a></div></div>}
  </section>;
}
