...
export function RespondentPortal({ item, token }: { item: MammusoCase; token: string }) {
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/case/respondent", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, statement, answers: {}, privacyPolicyAgreed, aiProcessingAgreed, strongLanguageAgreed }) });
  ...
  if (result) return <CaseResult result={result} />;
