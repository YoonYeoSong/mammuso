  ...
  if (phase === "love-chat") return <section className="tarot-shell love-chat-screen" aria-label="연애 타로 대화">
    ...
    <div className="chat-thread" aria-live="polite">
      ...
      {loveSituation && <>
        ...
        {showLoveQuestionPrompt && !isLoveQuestionSent && <div className="chat-enter chat-question-turn"><div className="chat-prompt-stack"><div className="chat-message bot"><span>타로킹</span><p>좋아.</p></div><div className="chat-message bot"><span>타로킹</span><p>마지막으로 무엇이 궁금한지 편하게 적어줘.</p></div><p className="chat-note">짧게 적어도 괜찮아.</p></div><label className="chat-question"><textarea value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="예: 썸 타는 사람이 있는데, 먼저 연락해도 될지 궁금해." maxLength={280} /></label><button className="chat-send" type="button" onClick={sendLoveQuestion} disabled={question.trim().length < 2}>보내기 <span>↑</span></button></div>}
  ...
  if (phase === "money-chat") return <section className="tarot-shell love-chat-screen" aria-label="돈 타로 대화">
    ...
    <div className="chat-thread" aria-live="polite">
      ...
      {moneyFocus && <>
        ...
        {showMoneyQuestionPrompt && !isMoneyQuestionSent && <div className="chat-enter chat-question-turn"><div className="chat-prompt-stack"><div className="chat-message bot"><span>타로킹</span><p>좋아.</p></div><div className="chat-message bot"><span>타로킹</span><p>마지막으로 무엇이 궁금한지 편하게 적어줘.</p></div><p className="chat-note">짧게 적어도 괜찮아.</p></div><label className="chat-question"><textarea value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="예: 올해 전반적인 재물운이 궁금해." maxLength={280} /></label><button className="chat-send" type="button" onClick={sendMoneyQuestion} disabled={question.trim().length < 2}>보내기 <span>↑</span></button></div>}
  ...
  if (phase === "picking") return <section className="tarot-shell pick-screen">
    ...
    {replacementCandidateId && (() => {
      ...
      return <div className="selection-confirm replacement-confirm" role="dialog" aria-modal="true" aria-labelledby="replacement-confirm-title"><div>
        {replacing ? <><p className="step">카드 교체 확인</p><h2 id="replacement-confirm-title">새 카드를<br />{replacementSlotNumber}번 자리에 넣을까?</h2><p>기존 카드는 다시 카드 더미로 돌아가요.</p><div><button type="button" className="secondary-action" onClick={() => setReplacementSlotIndex(null)}>아니오</button><button type="button" className="confirm-action" onClick={confirmReplacement}>응, 바꿀게</button></div></> : <><p className="step">카드 교체</p><h2 id="replacement-confirm-title">새 카드를<br />어느 자리에 넣을까?</h2><p>바꿀 자리만 먼저 골라줘.</p><div className="replacement-slots">{selectedCards.map((card, index) => <button key={card.id} type="button" className="replacement-slot" onClick={() => setReplacementSlotIndex(index)} aria-label={`${index + 1}번 자리 선택`}><b>{index + 1}</b><span>{index + 1}번 자리</span></button>)}</div><button type="button" className="replacement-close" onClick={closeReplacement}>닫기</button></>}</div></div>;
    })()}
