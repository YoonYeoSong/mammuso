...
export function TarotExperience() {
  ...
  if (phase === "picking") return <section className="tarot-shell pick-screen">
    ...
    <div className="card-spread" aria-label="22장 타로 카드">
        {deck.map((card, index) => {
          const selectIndex = selectedIds.indexOf(card.id);
          return <button key={card.id} type="button" className={`spread-card ${selectIndex >= 0 ? "is-selected" : ""}`} onClick={() => toggleCard(card.id)} aria-label={`카드 ${index + 1}${selectIndex >= 0 ? ", 선택됨" : ""}`}><CardFace card={card} revealed={false} /></button>;
        })}
    </div>
